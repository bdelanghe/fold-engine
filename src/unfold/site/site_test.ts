...
Deno.test("site build renders WebSite and WebPage fixtures", async () => {
  ...
  await copyFixture(websiteFixture, join(tempVaultDir, "web_site", "site.jsonld"));
  await copyFixture(
    webpageFixture,
    join(tempVaultDir, "web_pages", "hello.jsonld"),
  );
  ...
  const env = {
    ...
    WORKSPACE_ROOT: Deno.env.get("WORKSPACE_ROOT"),
  };
  ...
  try {
    ...
    Deno.env.set("WORKSPACE_ROOT", workspaceRoot);
    ...
    const rootIndexHtml = await Deno.readTextFile(rootIndex);
    assert(
      !rootIndexHtml.includes('http-equiv="refresh"'),
      "Expected root index to be a rendered page, not a redirect",
    );
    ...
    if (env.WORKSPACE_ROOT === undefined) {
      ...
      Deno.env.set("WORKSPACE_ROOT", env.WORKSPACE_ROOT);
    }
  }
});

Deno.test("createSite requires WORKSPACE_ROOT", () => {
  const env = {
    WORKSPACE_ROOT: Deno.env.get("WORKSPACE_ROOT"),
    SITE_URL: Deno.env.get("SITE_URL"),
    SITE_OUTPUT_DIR: Deno.env.get("SITE_OUTPUT_DIR"),
  ...
  try {
    Deno.env.delete("WORKSPACE_ROOT");
    Deno.env.set("SITE_URL", "https://example.org");
    Deno.env.set("SITE_OUTPUT_DIR", ".unfold/site");
    Deno.env.set("VAULT_PATH", "vault");
    assertThrows(() => createSite(), Error, "WORKSPACE_ROOT is required.");
  } finally {
    if (env.WORKSPACE_ROOT === undefined) {
      Deno.env.delete("WORKSPACE_ROOT");
    } else {
...
Deno.test("createSite requires SITE_OUTPUT_DIR", () => {
  const env = {
    WORKSPACE_ROOT: Deno.env.get("WORKSPACE_ROOT"),
    SITE_URL: Deno.env.get("SITE_URL"),
    SITE_OUTPUT_DIR: Deno.env.get("SITE_OUTPUT_DIR"),
  ...
  try {
    Deno.env.set("WORKSPACE_ROOT", Deno.cwd());
    Deno.env.set("SITE_URL", "https://example.org");
    Deno.env.delete("SITE_OUTPUT_DIR");
    Deno.env.set("VAULT_PATH", "vault");
    assertThrows(() => createSite(), Error, "SITE_OUTPUT_DIR is required.");
  } finally {
    if (env.WORKSPACE_ROOT === undefined) {
      Deno.env.delete("WORKSPACE_ROOT");
    } else {
...
Deno.test("createSite requires VAULT_PATH when vault missing", async () => {
  const env = {
    WORKSPACE_ROOT: Deno.env.get("WORKSPACE_ROOT"),
    SITE_URL: Deno.env.get("SITE_URL"),
    SITE_OUTPUT_DIR: Deno.env.get("SITE_OUTPUT_DIR"),
  ...
  try {
    Deno.env.set("WORKSPACE_ROOT", tempRoot);
    Deno.env.set("SITE_URL", "https://example.org");
    Deno.env.set("SITE_OUTPUT_DIR", ".unfold/site");
    Deno.env.delete("VAULT_PATH");
    assertThrows(
      () => createSite(),
      Error,
      "VAULT_PATH is required when /vault is missing.",
    );
  } finally {
    if (env.WORKSPACE_ROOT === undefined) {
      Deno.env.delete("WORKSPACE_ROOT");
