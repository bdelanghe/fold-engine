...
This document outlines the Linked Data standards and graph integrity features being added to Unfold.

## Overview

Unfold is evolving from a static site generator into a **deterministic Linked Data compiler** with formal graph integrity guarantees. This architecture document describes the W3C standards being adopted and the validation contracts being enforced.
...
for (const node of allNodes) {
  for (const edge of extractEdges(node)) {
    edges.push({ source: node['@id'], predicate: edge.type, target: edge.target });
  }
}
```
...
Primary chains:
- **Integrity:** Cool URIs → Link Resolver → Graph Validation → Reachability → Validation Modes
